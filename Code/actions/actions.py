    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        with PrologMQI(port=8000) as mqi:
            with mqi.create_thread() as prolog_thread:
                prolog_thread.query_async(r"consult('.\\actions\\PlanDeEstudio.pl')", find_all=False)
    ...
    def run(self, dispatcher: CollectingDispatcher, tracker: Tracker, domain: Dict[Text, Any]) -> List[Dict[Text, Any]]:
        ...
        with PrologMQI(port=8000) as mqi:
            with mqi.create_thread() as prolog_thread:
                ...
                if materiasEnCursada or str(materia)=="exploratoria":
        ...
        with PrologMQI(port=8000) as mqi:
            with mqi.create_thread() as prolog_thread:
                ...
                if result:
                    ...
                    prolog_thread.query_async(f"materia_en_cursada('{materia}')", find_all=False)
                    result=prolog_thread.query_async_result()
                    if result or str(materia)=="exploratoria":
